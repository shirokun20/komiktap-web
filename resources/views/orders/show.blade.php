...
<head>
    ...
    <title>Detail Pesanan {{ $transaction->code }} - KomikTap</title>
...
<body class="antialiased min-h-screen flex flex-col">
    ...
    <nav class="w-full bg-[#0f0e13]/80 backdrop-blur-xl border-b border-white/5 h-16 flex items-center">
        <div class="max-w-4xl mx-auto px-4 w-full flex justify-between items-center">
            ...
            <a href="{{ route('orders.index') }}" class="text-gray-400 hover:text-white text-sm flex items-center gap-2 transition-colors">
                <i class="fas fa-arrow-left text-xs"></i> Cek Pesanan
            </a>
        </div>
    </nav>
            ...
            <div class="glass-card rounded-2xl p-6 space-y-5">
                ...
                <div class="space-y-3 text-sm">
                    ...
                    <div class="flex justify-between">
                        ...
                        <span class="text-white font-mono text-xs">{{ $transaction->tripay_reference }}</span>
                    </div>
                    @endif
                    @if($transaction->tripay_paid_at)
                    <div class="flex justify-between">
                        <span class="text-gray-500">Dibayar Pada</span>
                        <span class="text-white">{{ \Carbon\Carbon::parse($transaction->tripay_paid_at)->format('d M Y, H:i') }}</span>
                    </div>
                    @endif
                </div>
                ...
                <div class="flex gap-3">
                    ...
                    <a href="{{ route('orders.index') }}"
                        ...
                        <i class="fas fa-search mr-1"></i> Cek Pesanan Lain
                    </a>
                </div>
